...
If found, remove the plaintext value and redeploy via `release-aliyun.yml`.
...
3. Trigger `release-aliyun.yml` (workflow_dispatch) to redeploy.
4. On host, verify the service started: `systemctl status ploy`.
...
3. Trigger `release-aliyun.yml` to redeploy.
4. Verify: `systemctl status ploy` and check logs for Grok API calls.
