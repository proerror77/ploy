...
| Research snapshot | `research_snapshot_compile` emits immutable sampled artifacts with source-surface metadata | Canonical factor-search input; not full-resolution replay evidence |
...
3. **Raw full-depth CLOB and sampled snapshot semantics are now explicit, but
   ...
   The manifest records `raw_full_fidelity` and `snapshot_sampled`, and the
   runbook says sampled snapshot is not full data. Any future report that uses
   full-depth execution language must cite candidate replay or full-depth lake
   evidence, not only snapshot rows.
