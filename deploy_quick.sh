...
echo ""
echo "✅ 部署完成！"
echo "訪問地址：http://$EC2_IP"
DEPLOY_SCRIPT

# 上傳部署腳本到 S3
aws s3 cp /tmp/deploy_on_ec2.sh s3://$BUCKET_NAME/ --quiet
echo "   ✅ 部署腳本創建完成"
...
if [ "$choice" = "1" ]; then
    ...
    if aws ssm describe-instance-information --filters "Key=InstanceIds,Values=$INSTANCE_ID" --query 'InstanceInformationList[0].PingStatus' --output text 2>/dev/null | grep -q "Online"; then
        ...
        COMMAND_ID=$(aws ssm send-command \
            ...
            --parameters "commands=['aws s3 cp s3://$BUCKET_NAME/deploy_on_ec2.sh /tmp/ && chmod +x /tmp/deploy_on_ec2.sh && /tmp/deploy_on_ec2.sh']" \
            --output text \
...
if [ "$choice" = "2" ]; then
    ...
    echo ""
    echo "2. 在 EC2 上執行："
    echo "   aws s3 cp s3://$BUCKET_NAME/deploy_on_ec2.sh /tmp/"
    echo "   chmod +x /tmp/deploy_on_ec2.sh"
    echo "   /tmp/deploy_on_ec2.sh"
    echo ""
fi
