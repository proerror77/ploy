...
fn main() {
    ...
    study.optimize(n_trials, move |trial: &mut Trial| {
        ...
        let config = make_config(min_prob, min_edge, cooldown, min_time, max_time);
        let (net_pnl, trades, sharpe) = run_backtest(config, &train_ref);

        eprintln!(
            "  Trial {:>3}: sharpe={:>7.3}  pnl=${:>8.2}  trades={:>4}  p={:.3}  edge={:.4}  cd={}s",
            trial.id(), sharpe, net_pnl, trades, min_prob, min_edge, cooldown
        );
