...
- Deployments that affect `tango-1-1`, trade services, or production-like state
  must run from `main`. Feature branches may run build-only or research-only
  workflows only when they do not mutate deployment state.
...
- Prefer environment-scoped workflows (`tango-1-1`, `production`, `ploy-ci-1`)
  over direct SSH when an equivalent workflow exists.
...
- When an issue leads to implementation, create or link a separate implementation
  issue/PR so research conclusions, code changes, and deployment decisions stay
  traceable.
