#!/bin/bash
# Backfill clob_quote_ticks using SQL-based approach (FIXED VERSION)
...
echo "=== Quote Data Backfill Script (Fixed) ==="
...
backfill_date_range() {
    ...
    # Generate quotes using SQL - FIXED: Use single CTE for both INSERTs
    psql "$DB_URL" <<EOF
    ...
    -- Prepare both UP and DOWN quotes in a single CTE
    all_quotes AS (
        -- UP token quotes
        SELECT
            ...
            'backfill_synthetic_v2' AS source,
            'crypto' AS domain
    ...
    FROM clob_quote_ticks
    WHERE source = 'backfill_synthetic_v2'
...
psql "$DB_URL" -c "DELETE FROM clob_quote_ticks WHERE source = 'backfill_synthetic';"
...
echo ""
echo "=== Token Coverage Check ==="
psql "$DB_URL" -c "
    SELECT
        COUNT(DISTINCT token_id) AS total_tokens,
        COUNT(DISTINCT CASE WHEN side = 'UP' THEN token_id END) AS up_tokens,
        COUNT(DISTINCT CASE WHEN side = 'DOWN' THEN token_id END) AS down_tokens
    FROM clob_quote_ticks
    WHERE source = 'backfill_synthetic_v2'
