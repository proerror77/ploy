...
1,auto_settlement_conservative_settlement_edge,tradeable_full_depth_settlement_pnl,candidate,passed,49831,0.110842,0.150273,43,1.064178,0.9535,6,0.8333,1.0000,9966,2.666226,0.6836,0.9000,9966,1,1
""")
'''
...
target labels are side-aligned executable settlement PnL; reports are candidate discovery gates, not deploy decisions.
rank,name,target,decision,reason,n,spearman_ic,pearson_ic,window_count,icir,positive_window_ratio,symbol_count,symbol_positive_ratio,monotonicity,top_bucket_n,top_bucket_avg_label,top_bucket_positive_label_rate,top_bucket_full_depth_entry_fill_rate,top_bucket_unique_event_count,top_bucket_max_event_decisions,complexity
1,mut_amplitude_weighted_momentum_30s_sigma_spread_adjusted,tradeable_full_depth_settlement_pnl,candidate,passed,3335,0.067525,0.081201,14,1.288053,0.9286,2,1.0000,1.0000,667,3.160059,0.6132,0.9000,667,1,6
2,auto_settlement_conservative_settlement_edge,tradeable_full_depth_settlement_pnl,candidate,passed,2798,0.067534,0.091002,12,1.018091,0.7500,2,1.0000,1.0000,560,2.507843,0.6250,0.9000,560,1,1
""")
'''
...
class FactorWalkForwardSweepTests(unittest.TestCase):
    ...
    def test_empty_snapshot_bound_sampling_args_resolve_from_manifest(self):
        with tempfile.TemporaryDirectory() as raw_tmp:
            tmp = Path(raw_tmp)
            (tmp / "snapshot").mkdir()
            (tmp / "snapshot" / "manifest.json").write_text(
                json.dumps(
                    {
                        "lob_sample_secs": 120,
                        "observation_sample_secs": 120,
        ...
        for flag in [
            ...
            self.assertEqual(captured[captured.index(flag) + 1], "120")
