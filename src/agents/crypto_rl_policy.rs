...
use crate::strategy::crypto_rl_policy::core as rl_policy_core;
use crate::strategy::crypto_rl_policy::core::{
    DiscreteAction, PositionLeg, TrackedPosition,
};
        ...
        let policy_model: Option<OnnxModel> = match config.policy_model_path.as_deref() {
            Some(path) if !path.trim().is_empty() => {
                ...
                let primary_dim = if primary_version == 1 {
                    rl_policy_core::OBS_DIM_V1
                } else {
                    rl_policy_core::OBS_DIM_V2
                };
                ...
                _ = decision_tick.tick() => {
                    ...
                    for coin in &self.config.coins {
                        ...
                        let obs = if obs_version == 2 {
                            rl_policy_core::build_observation_v2(
                            ...
                            rl_policy_core::build_observation_v1(
                                self.config.default_shares,
                                self.config.max_time_remaining_secs,
                                now,
                                spot.price,
                        ...
                        if let Some(model) = &self.policy_model {
                            ...
                            if let Some(o) = out.as_deref() {
                                if let Some(act) =
                                    rl_policy_core::action_from_policy_output(
                                        self.config.policy_output.as_str(),
                                        o,
                                    )
                                {
                                    policy_source = "onnx";
                                    ...
                                    if self.config.exploration_rate > 0.0 && rand::random::<f32>() < self.config.exploration_rate {
                                        action = rl_policy_core::ContinuousAction::new(
                                    ...
                                    let deployment_id =
                                        rl_policy_core::deployment_id_for_symbol(&symbol);
                                    ...
                                    match discrete {
                                        ...
                                        DiscreteAction::BuyUp => {
                                            ...
                                            let shares = rl_policy_core::compute_shares(
                                                &action,
                                                self.config.default_shares,
                                            );
                        ...
                        let base_action = rl_policy_core::rule_based_policy(
                            has_pos,
                            Some(up_ask + down_ask),
                            momentum_1s,
                            unrealized,
                        );
                        ...
                        let deployment_id = rl_policy_core::deployment_id_for_symbol(symbol.as_str());

                        match discrete {
                            DiscreteAction::BuyUp => {
                                if has_pos || up_ask > self.config.max_entry_price { continue; }
                                let shares =
                                    rl_policy_core::compute_shares(&base_action, self.config.default_shares);
                                let mut intent = OrderIntent::new(
                                    &self.config.agent_id,
                                    Domain::Crypto,
                                    event.slug.as_str(),
                    ...
                    match cmd {
                        ...
                        Some(CoordinatorCommand::ForceClose) => {
                            ...
                            for (slug, pos) in &positions {
                                let deployment_id = rl_policy_core::deployment_id_for_symbol(&pos.symbol);
                                for leg in &pos.legs {
                                    let bid = quote_cache
                                        .get(&leg.token_id)
