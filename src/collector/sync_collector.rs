...
use super::token_targets::CollectorTokenTarget;
...
impl SyncCollector {
    ...
    pub async fn upsert_token_targets(&self, targets: &[CollectorTokenTarget]) -> Result<()> {
        self.persist_token_targets(targets).await
    }
    ...
    async fn process_lob_update(&self, update: LobUpdate) {
        ...
        if let Some(pool) = &self.pool {
            if let Err(e) = self.persist_binance_lob_tick(pool, &update).await {
                debug!("Failed to save raw Binance LOB tick: {}", e);
            }

            if self.persist_sync_records {
                if let Err(e) = self.persist_sync_record(pool, &record).await {
