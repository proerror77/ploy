...
impl LiveWinProbModel {
    ...
    pub fn predict(&self, features: &GameFeatures) -> WinProbPrediction {
        ...
        let logit = coef.intercept
            ...
            + coef.point_diff_x_quarter4 * (features.point_diff * if features.quarter == 4 { 1.0 } else { 0.0 });
...
mod tests {
    ...
    fn test_predict_basic() {
        ...
        assert!(prediction.win_prob > 0.7, "Win prob should be > 70% when ahead by 10 in Q4");
        assert!(prediction.confidence > 0.5, "Confidence should be reasonable in Q4");
    }
    ...
    fn test_predict_close_game() {
        ...
        assert!((prediction.win_prob - 0.5).abs() < 0.2, "Tied game should be close to 50%");
        assert!(prediction.uncertainty > 0.15, "Early game should have higher uncertainty");
    }
    ...
    fn test_uncertainty_increases_early() {
        ...
        assert!(pred_q1.uncertainty > pred_q4.uncertainty, "Q1 should have higher uncertainty than Q4");
    }
