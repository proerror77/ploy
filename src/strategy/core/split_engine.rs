...
impl SplitArbEngine {
    ...
    pub async fn on_price_update(&self, token_id: &str, bid: Option<Decimal>, ask: Option<Decimal>) {
        // Update cache
        {
    ...
    async fn check_for_entry(&self, market: &BinaryMarket) {
        ...
        let (side, entry_price, token_id, other_token, label, other_label) = 
            if yes_ask <= self.config.max_entry_price {
                (ArbSide::Yes, yes_ask, &market.yes_token_id, &market.no_token_id, 
                 &market.yes_label, &market.no_label)
            } else if no_ask <= self.config.max_entry_price {
                (ArbSide::No, no_ask, &market.no_token_id, &market.yes_token_id,
                 &market.no_label, &market.yes_label)
            } else {
        ...
        let other_ask = if side == ArbSide::Yes { no_ask } else { yes_ask };
        ...
        // Execute entry
        if self.dry_run {
            info!("  [DRY RUN] Would buy {} shares of {}", self.config.shares_per_trade, label);
        } else {
            // Place order (simplified - full implementation would use executor)
            info!("  Placing order for {} shares of {}", self.config.shares_per_trade, label);
        }
