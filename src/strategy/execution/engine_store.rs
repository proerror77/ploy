...
pub mod mock {
    ...
    impl EngineStore for MockStore {
        ...
        async fn update_cycle_state(
            ...
            if self.compare_and_bump_cycle_version(cycle_id, expected_version) {
                Ok(())
            } else {
                Err(crate::error::PloyError::VersionConflict {
        ...
        assert!(
            matches!(
                ...
                Err(crate::error::PloyError::VersionConflict {
                    ...
                    id,
                    expected_version
                }) if entity == "cycle" && id == cycle_id && expected_version == 1
            ),
            "stale expected_version must raise explicit version conflict"
        );
    }
}
