...
pub struct EventMatcher {
    ...
    pub(crate) symbol_to_series: HashMap<String, Vec<String>>,
    /// Cache of active events per series
    pub(crate) active_events: Arc<RwLock<HashMap<String, Vec<EventInfo>>>>,
}
...
impl EventMatcher {
    ...
    pub fn new(client: PolymarketClient) -> Self {
        ...
        symbol_to_series.insert("BTCUSDT".into(), vec!["10684".into(), "10192".into()]);
        symbol_to_series.insert("ETHUSDT".into(), vec!["10683".into(), "10191".into()]);
        symbol_to_series.insert("SOLUSDT".into(), vec!["10686".into(), "10423".into()]);
        symbol_to_series.insert("XRPUSDT".into(), vec!["10685".into(), "10422".into()]);
    ...
    async fn fetch_series_events(&self, series_id: &str) -> Result<Vec<EventInfo>> {
        ...
        for gamma_event in sorted_events.into_iter().take(5) {
            ...
            let is_up_label = |label: &str| -> bool {
                let outcome = label.trim().to_ascii_lowercase();
                outcome == "up"
                    || outcome.contains("up")
                    || outcome == "yes"
                    || outcome.starts_with('↑')
            ...
            if let Some(token_infos) = market.tokens.as_ref() {
                for token in token_infos {
                    if up_token_id.is_none() && is_up_label(&token.outcome) {
                        up_token_id = Some(token.token_id.clone());
                    }
                    if down_token_id.is_none() && is_down_label(&token.outcome) {
                    ...
                    if let Ok(outcomes) = serde_json::from_str::<Vec<String>>(outcomes_raw) {
                        if outcomes.len() == token_ids.len() {
                            ...
                            if let Some(idx) = up_idx {
                                up_token_id = Some(token_ids[idx].clone());
                            }
                            if let Some(idx) = down_idx {
                                down_token_id = Some(token_ids[idx].clone());
                            }
                        }
                    }
                }
    ...
    fn convert_gamma_event(&self, gamma: &GammaEventInfo) -> Option<EventInfo> {
        ...
        for token in tokens {
            debug!("  Token: {} = {}", token.token_id, token.outcome);
        }

        let up_token = tokens.iter().find(|token| {
            let outcome = token.outcome.to_lowercase();
            outcome.contains("up") || outcome == "yes" || outcome.starts_with("↑")
        })?;
        let down_token = tokens.iter().find(|token| {
            let outcome = token.outcome.to_lowercase();
