...
use crate::domain::{Domain, OrderStatus, OrderType, Side, TimeInForce};
...
impl StaggeredArbAdapter {
    ...
    fn try_entry_for_window(
        ...
        if self.dry_run {
            ...
            *self
            ...
            metadata.insert("event_id".to_string(), window.event_id.clone());
            metadata.insert("symbol".to_string(), symbol.to_string());
            metadata.insert("leg".to_string(), "1".to_string());
            if let Some(condition_id) = &window.condition_id {
                metadata.insert("condition_id".to_string(), condition_id.clone());
            }
            ...
            Some(StrategyAction::SubmitIntent {
                intent: StrategyOrderIntent {
                    ...
                    market_slug: window.event_id.clone(),
                    token_id: token_id.clone(),
                    side,
                    is_buy: true,
                    shares,
                    limit_price: leg1_ask,
    ...
    fn fill_leg2(
        ...
        if self.dry_run {
            ...
            let mut metadata = HashMap::new();
            metadata.insert("event_id".to_string(), event_id);
            metadata.insert("symbol".to_string(), symbol);
            metadata.insert("leg".to_string(), "2".to_string());
            metadata.insert("close_reason".to_string(), reason.to_string());
            if let Some(condition_id) = &pos.condition_id {
                metadata.insert("condition_id".to_string(), condition_id.clone());
            }
            ...
            Some(StrategyAction::SubmitIntent {
                intent: StrategyOrderIntent {
                    client_order_id,
                    domain: Domain::Crypto,
                    market_slug: pos.event_id.clone(),
                    token_id,
                    side,
                    is_buy: true,
                    shares,
                    limit_price: other_ask,
                    order_type: OrderType::Limit,
                    time_in_force: TimeInForce::GTC,
                    priority: 10,
                    metadata,
