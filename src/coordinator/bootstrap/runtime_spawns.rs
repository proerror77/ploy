...
pub(super) async fn spawn_managed_strategy_runtime_task(
    ...
    let strategy_cmd_rx =
        coordinator.register_agent(agent_id.clone(), spec.domain, spec.risk_params).await;
...
pub(super) async fn spawn_openclaw_governance_agent(
    ...
    let cmd_rx = coordinator.register_agent(oc_agent_id.clone(), Domain::Custom(0), oc_risk_params).await;
