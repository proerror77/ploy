...
mod tests {
    ...
    #[test]
    fn openclaw_config_defaults_match_existing_bootstrap_expectations() {
        let cfg = OpenClawConfig::default();
        assert_eq!(cfg.agent_id, "openclaw");
        assert!(!cfg.enabled);
        assert_eq!(cfg.regime_tick_secs, 15);
        assert_eq!(cfg.perf_tick_secs, 30);
        assert_eq!(cfg.alloc_tick_secs, 120);
        assert_eq!(cfg.perf_window_secs, 3600);
        assert_eq!(cfg.btc_symbol, "BTCUSDT");
    }
}
