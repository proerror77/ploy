...
pub(super) async fn ensure_clob_quote_ticks_table(pool: &PgPool) -> Result<()> {
    crate::platform::persistence_schema::ensure_clob_quote_ticks_table(pool).await
}

pub(super) async fn ensure_binance_price_ticks_table(pool: &PgPool) -> Result<()> {
    crate::platform::persistence_schema::ensure_binance_price_ticks_table(pool).await
}

pub(super) async fn ensure_binance_lob_ticks_table(pool: &PgPool) -> Result<()> {
...
pub(super) async fn ensure_accounts_table(pool: &PgPool) -> Result<()> {
    sqlx::query(
        r#"
...
pub(super) async fn upsert_account_from_config(
    pool: &PgPool,
    account_id: &str,
...
pub(super) async fn ensure_schema_repairs(pool: &PgPool) -> Result<()> {
