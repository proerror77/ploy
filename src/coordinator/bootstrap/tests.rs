...
use crate::control_plane::{
    DeploymentExecutionMode, MarketSelector, StrategyLifecycleStage, StrategyProductType, Timeframe,
};
...
fn build_pm_5m_directional_runtime_config_renders_symbol_filters() {
    let rendered = build_pm_5m_directional_runtime_config(&[
        "ETH".to_string(),
        "BTCUSDT".to_string(),
    ])
...
fn build_crypto_lob_ml_runtime_config_renders_coin_filters() {
    ...
    let rendered = build_crypto_lob_ml_runtime_config(&cfg).expect("render crypto_lob_ml config");
...
fn build_event_edge_runtime_config_rejects_empty_targets() {
    ...
    let err = build_event_edge_runtime_config(&cfg).expect_err("event_edge config without targets");
    assert!(
        err.to_string()
...
fn build_nba_comeback_runtime_config_renders_risk_controls() {
    ...
    let value: toml::Value = toml::from_str(&rendered).expect("valid nba_comeback runtime toml");
