...
pub struct CoordinatorHandle {
    ...
    // Stub fields for removed functionality (kept for API compatibility)
...
impl Coordinator {
    ...
    async fn apply_kelly_sizing(&self, intent: &mut OrderIntent) -> Option<String> {
        ...
        let bankroll = intent.notional_value();
    ...
    async fn check_duplicate_intent(&self, intent: &OrderIntent) -> Option<String> {
        let mut guard = self.admission.duplicate_guard.write().await;
    ...
    fn stable_idempotency_key(
        ...
        let dep_label = match scope {
            ...
            DuplicateGuardScope::Deployment => intent_deployment_scope(intent),
        };
