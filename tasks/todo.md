...
- Review update (2026-04-22): Addressed the `reconcile_fills` N+1 review item. Live Polymarket reconciliation now deduplicates tracked orders by token id, fetches each token's paginated trade stream concurrently, then applies the existing order-level fill filter locally. This preserves reconciliation semantics while avoiding duplicate sequential API calls for multiple orders on the same token.
