...
- [x] Push PR updates and run remote replay with diagnostics; decide next strategy change from bucket evidence.
- [x] Add a direction-side filter to test `both` / `up_only` / `down_only` as an explicit EV factor.
- [x] Rerun remote replay with side-filter search and compare filled validation EV.
- [x] Add entry-price-floor search to avoid repeatedly losing cheap-token buckets.
- [ ] Rerun remote replay with entry-price floor search and compare filled validation EV.
...
- 2026-04-30: PR #254 Test run `25151885517` passed after adding diagnostics. Remote research-only optimize run `25152002767` succeeded on branch `a09a779e` with official settlement and LOB liquidity. Best validation stayed negative: PnL `-$162.69`, `17` trades, Sharpe `-164.01`. Worst validation buckets were `edge=>=0.10` `-$162.69/17`, `p_hat=>=0.70` `-$162.69/17`, and `direction=DOWN` `-$153.19/13`; this is not deployable. The next correction is to test direction-side filtering as a first-class EV factor, not to loosen win-rate or probability thresholds blindly.
- 2026-04-30: Added `three_layer_allowed_directions` as a backward-compatible empty-default strategy field, plus optimizer-side `three_layer_direction_filter` search over `both`, `up_only`, and `down_only`. Direction probability remains a hard gate; the new field only asks which side should be eligible after the directional alpha/EV gates pass. Local verification passed: `rtk cargo test -p ploy-strategy-bundles --example optimize_backtest` (`6 passed`), `rtk cargo test -p ploy-strategy-bundles three_layer --lib` (`38 passed, 103 filtered out`), `rtk cargo test -p ploy-strategy-bundles config --lib` (`18 passed, 123 filtered out`), `rtk cargo test -p ploy-strategy-bundles --test backtest_integration` (`6 passed`), `rtk cargo check -p ploy-strategy-bundles --features parquet-feed --example optimize_backtest` (`0 errors`, existing warnings), and `git diff --check`.
- 2026-04-30: Side-filter optimizer run `25152488579` succeeded on branch `52856123` with 9 full replay trials. Best validation selected `up_only` but still failed: PnL `-$77.62`, `8` trades, Sharpe `-179.23`. Down-only trials were worse but more powered (`30-41` validation trades, all negative). Best validation buckets remained negative: `direction=UP` `-$77.62/8`, `edge=>=0.10` `-$77.62/8`, `symbol=BTCUSDT` `-$60.80/4`, and `entry_price=<0.35` `-$45.68/3`. Direction-side filtering reduces exposure but does not create a deployable edge.
- 2026-04-30: Added `three_layer_min_entry_price` to the optimizer search by wiring the existing `min_entry_price` runtime field into `ThreeLayerSearchParams`. This specifically tests whether removing repeatedly losing cheap-token fills can improve executable EV; it does not change existing configs unless the optimizer outputs a higher `min_entry_price`.
- 2026-04-30: Local verification passed after adding entry-price-floor search: `rustfmt --edition 2024 crates/ploy-strategy-bundles/examples/optimize_backtest.rs`, `CARGO_TARGET_DIR=/tmp/ploy-bayesian-ev-gate-test rtk cargo test -p ploy-strategy-bundles --example optimize_backtest` (`6 passed`), `CARGO_TARGET_DIR=/tmp/ploy-bayesian-ev-gate-test rtk cargo check -p ploy-strategy-bundles --features parquet-feed --example optimize_backtest` (`0 errors`, existing warnings), and `git diff --check`.
